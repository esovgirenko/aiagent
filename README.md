...
- Для queued цели можно задать `iterations`: worker повторяет цикл до валидного результата
  (`PASS` + результат не `НЕТ ДАННЫХ`) или пока не достигнет лимита итераций.
- В admin UI добавлена кнопка очистки очереди.
- Цикл теперь пытается выполнить предложенное действие:
  - `llm_query` (выполняет запрос к LLM),
  - `shell_safe` (ограниченный whitelist безопасных команд чтения).
- Оркестратор очереди использует:
